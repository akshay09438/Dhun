...
describe("App — the catalog flow", () => {
  ...
  it("hides the mix style and take counter from users", async () => {
    mockBackend();
    render(<App />);
    await pickBothSongs();
    fireEvent.click(screen.getByRole("button", { name: /mix it/i }));

    // Wait for the Play screen (regenerate appears for a single mix), then confirm the internal
    // mix STYLE and the TAKE number are NOT shown to users anywhere.
    await screen.findByRole("button", { name: /regenerate/i });
    expect(screen.queryByTestId("rule-badge")).toBeNull();
    expect(screen.queryByTestId("take-count")).toBeNull();
    expect(screen.queryByText(/mix style/i)).toBeNull();
    expect(screen.queryByText(/take \d+ of \d+/i)).toBeNull();
    // No manual rule buttons anywhere either.
    expect(screen.queryByTestId("rule-1")).toBeNull();
    expect(screen.queryByTestId("rule-4")).toBeNull();
  });

  it("caps regeneration at 5 generations per sitting", async () => {
    mockBackend();
    render(<App />);
    await pickBothSongs();
    fireEvent.click(screen.getByRole("button", { name: /mix it/i }));

    // Regenerate up to the cap; after the 5th generation the button is gone. The cap still holds
    // even though the take number is no longer shown to users.
    for (let i = 2; i <= 5; i++) {
      const btn = await screen.findByRole("button", { name: /regenerate/i });
      fireEvent.click(btn);
      await waitFor(() => {
        const b = screen.queryByRole("button", { name: /regenerate/i });
        expect(b === null || !(b as HTMLButtonElement).disabled).toBe(true);
      });
    }
  ...
  it("stacks two sets → builds a continuous set → Play shows both back-to-back", async () => {
    ...
    // The per-mix STYLE is internal-only now — the line-up shows song names + "Set N", not the rule.
    expect(screen.queryByTestId("lineup-rule-1")).toBeNull();
    expect(screen.queryByTestId("lineup-rule-2")).toBeNull();
  });
