...
## Status: **M1 complete; M2a (stem splitting) complete — 23 tests passing, live cloud split verified.** Next: M2b (beat/key/structure analysis).
...
| M2b | Analysis            | `TrackAnalysis` (BPM/beat/downbeats/key/sections/energy/vocal-regions) with per-field confidence, cached; overlays in UI    | BPM ±1 and downbeats on the one for demo pairs                               | ☐       |
