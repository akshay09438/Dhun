...
problem is neither: **about one in six catalog pairs fails outright**, today, with one user.
...
It is concentrated, which is the good news — this is a handful of songs, not a systemic rot:
...
**Two beats (Innerbloom, Rapture) account for 15 of the 17 failures, and one vocal (Khuda Jaane)
has never once succeeded.** Fixing or withdrawing those three songs would remove most of the
problem immediately.

**Honest limits of this number.** Only 4 of the 12 beats were reached — Anchor Point, Merrygo, Wake
Me Up, Faded, Lean On, Closer, Hey Brother and Silence are **untested**, and the guest-verse beats
among them are exactly the ones with unusual handling, so the true rate could move in either
direction. The sweep also lost its own summary file when it was stopped; the numbers above were
reconstructed from the engine's `events.db`, which records every render outcome and is the more
trustworthy source anyway. Re-running the remaining 134 pairs needs about 16 GB of free disk, or a
change to delete each render as soon as its result is known.
...
|---|---|---|---|
| 1 | **~1 in 5 pairs fails outright** | **Yes, today, with one user** | No |
| 2 | **One voice stream per server** — 10 rooms, 9 silent | **Yes, structural** | No |
| 3 | **No admission control** — a big burst exhausts memory and takes everyone down | **Yes, untested past 10** | Partly |
| 4 | **No queue position shown** — "grinding…" whether it is 30s or 5 min away | Yes | No |
| 5 | Rendering speed | **No. Already ~5.5x parallel** | n/a |
...
**1. Fix or hide the failing pairs.** Nothing else matters if a fifth of what people try dies, and
it is concentrated in three songs. Two complementary routes: make the vocal chain back off when it
is about to collapse the crest factor (the real fix), and stop offering a pairing that cannot work
(a cheap guard). Withdrawing or repairing **Khuda Jaane, Innerbloom and Rapture** alone would clear
most of it. Finish the sweep first so the list is complete rather than half-known.

**2. Put a limit on simultaneous renders.** Not to make it faster — to stop it falling over. A
semaphore of about 6–8 on this machine turns a 50-person spike from "everyone loses their work"
into "everyone waits a bit." This is the single highest-value engine change and it is small.

**3. Show queue position.** Ten people staring at "grinding…" with no idea whether it is 30 seconds
...
**4. Keep two or three listening rooms, not ten.** With one voice connection per server, ten rooms
...
**5. Move to a normal x86 server (~$32/month).** Two wins in one: voice playback starts working at
...
**6. Automatic deletion of old mixes.** A precondition, not an optimisation. Each finished mix is
...
- **Whether any pair fails ONLY under load.** Every failure sampled so far also fails alone.
