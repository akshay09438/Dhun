...
> **Recovery + re-verify (2026-07-07, later):** the previous Claude Code process exited mid-task (it left one background research agent — "more Father Ocean 10B vocals" — orphaned; that work is **not needed**, the shelf's 4 vocals are already finalized). On re-open I confirmed **nothing was lost**: working tree clean, `feat/m5-live-control` in sync with origin, all work committed at `ed03c0d`. Re-ran the full suite — **181 backend + 39 web passed, typecheck clean** (evidence below). **No new code this session.** The servers + ngrok link that the prior session started are now **DOWN** (they were its children and died with it) — see "In flight" for revival.
...
- **Servers + link are DOWN right now** (confirmed 2026-07-07: `:8000` and `:5173` both refuse connections; ngrok tunnel closed with the prior session). **To bring the shareable link back up: founder double-clicks `Start-PromptDJ.bat`** (repo root) and keeps the window open. For local dev only: backend `uvicorn` on :8000 + `npm run dev` on :5173.
